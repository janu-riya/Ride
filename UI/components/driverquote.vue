<template>
              ...
              <v-row>
                ...
                <v-col>
                  <v-list-item>
                    ...
                    <v-list-item-content>
                      <v-list-item-title>{{ quote.mobile_number}}</v-list-item-title>
  ...
  export default {
    async mounted(){
        ...
        this.quote = this.$storage.getUniversal('driver_quote')
        console.log(this.quote)
             
        let url = "http://127.0.0.1:8000/quote/driver/all"
        let res = await this.$axios.get(url,{params:{driver_number: this.quote}})
